...
namespace mg
{
	expression::value_type expression::parse(const string_type &str, const function_dependencies &deps)
	{
		...
		static const std::string operator_search_pattern = get_pattern(
			R"((^${unary})|(${binary}))",
			{
		...
		for (auto i = begin; i != end; ++i, ++index)
		{
			...
			if (match[0].matched && match[1].matched && match[2].matched)
			{ // function match
				auto args_begin = boost::sregex_iterator(match[2].str().begin(), match[2].str().end(), split_args_rgx);
				...
				objects.emplace(
					str_index, std::make_unique<call_function_action>(deps.at(match[1].str()), std::move(function_args))
				);
			}
			if (match[3].matched && match[4].matched)
			{ // bracket expression (inner expression)
				objects.emplace(str_index, match[4].str());
			}
			if (match[5].matched)
			{ // variable matched
				string_type group = match[5].str();
				if (group != matching)
				{
			...
			if (match[6].matched)
			{ // number matched
				objects.emplace(str_index, number{ match[6].str() });
			}
		...
		for (auto op = op_begin; op != op_end; ++op)
		{
			...
			try
			{
				if (match[1].matched)
				{
					operations.push_back(&binary_operation::get_by_name(match.str()[0]));
				}
				if (match[0].matched)
				{
					operations.push_back(&unary_operation::get_by_name(match.str()[0]));
				}
			}
		...
		for (auto op = operations.begin(); op != operations.end(); op = operations.erase(op))
		{
			...
			if (auto binary = dynamic_cast<const binary_operation *>(*op))
			{
				...
				if (!(match[0].matched && match[1].matched))
				{
					throw std::runtime_error((match[0].matched ? string_type("right") : "left") + " operand not found");
				}
				objects.erase(objects.find(match[1].str()));
				str_index		= match[0].str();
				objects.at(str) = expression(
					std::make_unique<binary_operator_action>(objects.at(match[0]), *binary, objects.at(match[1]))
				);
			}
