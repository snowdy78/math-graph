...
TEST_CASE("operator_action construct", "[test]")
{
	...
	SECTION("construct by incorrect string")
	{
		...
		{
			REQUIRE_THROWS(mg::operator_action{ "" });
		}
		SECTION("wrong string")
		{
			REQUIRE_THROWS(mg::operator_action{ "a@43$FSA_fas" });
		}
		SECTION("too many operations")
		{
			REQUIRE_THROWS(mg::operator_action{ "1 + 2 + 3" });
		}
		SECTION("wrong operands")
		{
			REQUIRE_THROWS(mg::operator_action{ "1 + 2a" });
			REQUIRE_THROWS(mg::operator_action{ "$fa + 2" });
		}
		SECTION("less operands")
		{
			REQUIRE_THROWS(mg::operator_action{ "1 + " });
			REQUIRE_THROWS(mg::operator_action{ "+" });
			REQUIRE_THROWS(mg::operator_action{ "- 1" });
		}
		SECTION("wrong operation")
		{
			REQUIRE_THROWS(mg::operator_action{ "1 _ 2" });
			REQUIRE_THROWS(mg::operator_action{ "1 a 2" });
		}
		SECTION("negative var")
		{
			REQUIRE_THROWS(mg::operator_action{ "-x + 1" });
			REQUIRE_NOTHROW(mg::operator_action{ "+1 - x" });
		}
	}
	SECTION("construct by values")
	{
		SECTION("two numbers")
		{
			mg::operator_action op{ 1.0, mg::unique_operations::add, 2.0 };
			REQUIRE(op.op() == mg::unique_operations::add);
			REQUIRE(op.deps().size() == 0);
			REQUIRE(std::holds_alternative<mg::operator_action::number_type>(op.left()));
			REQUIRE(std::holds_alternative<mg::operator_action::number_type>(op.right()));
		...
		{
			mg::operator_action::variable_type x{ "x" };
			mg::operator_action op{ 1.0, mg::unique_operations::add, x };
			REQUIRE(op.op() == mg::unique_operations::add);
			REQUIRE(op.deps().size() == 1);
			REQUIRE(op.deps().contains({ "x" }));
			REQUIRE(std::holds_alternative<mg::operator_action::number_type>(op.left()));
		...
		{
			mg::operator_action::variable_type x{ "x" };
			mg::operator_action op{ x, mg::unique_operations::add, 1.0 };
			REQUIRE(op.op() == mg::unique_operations::add);
			REQUIRE(op.deps().size() == 1);
			REQUIRE(op.deps().contains({ "x" }));
			REQUIRE(std::holds_alternative<mg::operator_action::variable_type>(op.left()));
			REQUIRE(std::holds_alternative<mg::operator_action::number_type>(op.right()));
			REQUIRE(std::get<mg::operator_action::number_type>(op.right()) == 1.0);
		}
		SECTION("var and var")
		{
			mg::operator_action::variable_type x{ "x" }, x1{ "x1" };
			mg::operator_action op{ x, mg::unique_operations::add, x1 };
			REQUIRE(op.op() == mg::unique_operations::add);
			REQUIRE(op.deps().size() == 2);
			REQUIRE(op.deps().contains({ "x" }));
			REQUIRE(op.deps().contains({ "x1" }));
		...
		{
			REQUIRE_THROWS(mg::operator_action{ 1.0, mg::unique_operations::add, nullptr });
		}
		SECTION("number and other operation")
		{
			...
			REQUIRE(op.deps().contains({ "x" }));
			REQUIRE(std::holds_alternative<mg::operator_action::number_type>(op.left()));
			REQUIRE(std::holds_alternative<mg::operator_action::pointer_type>(op.right()));
			REQUIRE(std::get<mg::operator_action::number_type>(op.left()) == 1.0);
		}
