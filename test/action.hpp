#include <catch2/catch_test_macros.hpp>
#include "math_graph/action.hpp"

TEST_CASE("action construct", "[test]")
{
	SECTION("construct by binary_operator_action")
	{
		mg::binary_operator_action op{ "x + y" };
		mg::action act{ op };
		REQUIRE(act.is_operator_action() == true);
		REQUIRE(act.deps().size() == 2);
		REQUIRE(act.deps().contains({ "x" }));
		REQUIRE(act.deps().contains({ "y" }));
		REQUIRE_NOTHROW(act.as_binary_operation());
		REQUIRE_THROWS(act.as_function());
		REQUIRE_THROWS(act.as_unary_operation());
	}
	SECTION("construct by function")
	{
		mg::unexpressed_function op{ "f(x)" };
		mg::action act{ op };
		REQUIRE(act.is_function() == true);
		REQUIRE(act.deps().size() == 1);
		REQUIRE(act.deps().contains({ "x" }));
		REQUIRE_NOTHROW(act.as_function());
		REQUIRE_THROWS(act.as_binary_operation());
		REQUIRE_THROWS(act.as_unary_operation());
	}

	SECTION("construct by unary_operator_action")
	{
		mg::unary_operator_action op{ "-x" };
		mg::action act{ op };
		REQUIRE(act.is_unary_operation() == true);
		REQUIRE(act.deps().size() == 1);
		REQUIRE(act.deps().contains({ "x" }));
		REQUIRE_NOTHROW(act.as_unary_operation());
		REQUIRE_THROWS(act.as_function());
		REQUIRE_THROWS(act.as_binary_operation());
	}
}
